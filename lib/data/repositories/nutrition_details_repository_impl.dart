...
class NutritionDetailsRepositoryImplementation implements NutritionDetailsRepository {
  NutritionApiService _nutritionApiService;

  NutritionDetailsRepositoryImplementation(this._nutritionApiService);

  @override
  Future<DataState<DataResponseModel>> getNutritionDetails(IngredientRequestParams params) async {
    try {
      final httpResponse = await _nutritionApiService.getNutritionDetails(ingredientRequestParams: params);
      if (httpResponse == null) {
        return DataFailed(const HttpException("Wrong Data Entered"));
      }
      if (httpResponse.runtimeType == http.Response) {
        return DataSuccess(DataResponseModel.fromJson(json.decode((httpResponse as http.Response).body)));
      } else {
        return DataFailed(HttpException(httpResponse.response()));
      }
